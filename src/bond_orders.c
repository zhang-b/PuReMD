...
inline real Cf45(real p1, real p2) {
	return -EXP(-p2 / 2) / (SQR( EXP(-p1 / 2) + EXP(p1 / 2) ) * (EXP(-p2 / 2)
			+ EXP(p2 / 2)));
}
...
void Add_dBond_to_Forces_NPT(int i, int pj, reax_system *system,
	...
	coef.C1dDelta = bo_ij->C1dbo * (workspace->CdDelta[i]
			+ workspace->CdDelta[j]);
	coef.C2dDelta = bo_ij->C2dbo * (workspace->CdDelta[i]
			+ workspace->CdDelta[j]);
	coef.C3dDelta = bo_ij->C3dbo * (workspace->CdDelta[i]
			+ workspace->CdDelta[j]);
...
void Add_dBond_to_Forces(int i, int pj, reax_system *system,
	...
	/*1st, dBO*/
	rvec_ScaledAdd(system->atoms[i].f, coef.C2dDelta,
			workspace->dDeltap_self[i]);
	/*2nd, dBO*/
	...
	/*2nd, dBO_pi2*/
	rvec_ScaledAdd(system->atoms[i].f, coef.C3dbopi2,
			workspace->dDeltap_self[i]);
	/*3rd, dBO_pi2*/
	...
	/*1st, dBO*/
	rvec_ScaledAdd(system->atoms[j].f, coef.C3dDelta,
	...
	/*2nd, dBOpi2*/
	rvec_ScaledAdd(system->atoms[j].f, coef.C4dbopi2,
			workspace->dDeltap_self[j]);
	/*3rd, dBOpi2*/
}
...
void Calculate_Bond_Orders(reax_system *system, control_params *control,
			...
			if (i < j) {
				...
				if (twbp->ovc < 0.001 && twbp->v13cor < 0.001) {
					...
					if (twbp->ovc >= 0.001) {
						...
						f1 = 0.5 * ((val_i + f2) / (val_i + f2 + f3) + (val_j
								+ f2) / (val_j + f2 + f3));
						...
						u1_ij = val_i + temp;
						u1_ji = val_j + temp;
						Cf1A_ij = 0.5 * f3 * (1.0 / SQR( u1_ij ) + 1.0
								/ SQR( u1_ji ));
						Cf1B_ij = -0.5 * ((u1_ij - f3) / SQR( u1_ij ) + (u1_ji
								- f3) / SQR( u1_ji ));

						//Cf1_ij = -Cf1A_ij * p_boc1 * exp_p1i +
						//          Cf1B_ij * exp_p2i / ( exp_p2i + exp_p2j );
						Cf1_ij = 0.50 * (-p_boc1 * exp_p1i / u1_ij - ((val_i
								+ f2) / SQR(u1_ij)) * (-p_boc1 * exp_p1i
								+ exp_p2i / (exp_p2i + exp_p2j)) + -p_boc1
								* exp_p1i / u1_ji - ((val_j + f2) / SQR(u1_ji))
								* (-p_boc1 * exp_p1i + exp_p2i / (exp_p2i
										+ exp_p2j)));

						Cf1_ji = -Cf1A_ij * p_boc1 * exp_p1j + Cf1B_ij
								* exp_p2j / (exp_p2i + exp_p2j);
	...
	for (j = 0; j < system->N; ++j) {
		...
		workspace->Delta_e[j] = workspace->total_bond_order[j]
				- sbp_j->valency_e;
		workspace->Delta_boc[j] = workspace->total_bond_order[j]
				- sbp_j->valency_boc;

		workspace->vlpex[j] = workspace->Delta_e[j] - 2.0
				* (int) (workspace->Delta_e[j] / 2.0);
		...
		if (sbp_j->mass > 21.0) {
			...
			workspace->Delta_lp_temp[j] = sbp_j->nlp_opt
