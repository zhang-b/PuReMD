...
void Init_Taper(control_params *control) {
	...
	if (swb < 0) {
		...
		fprintf(stderr,
				"Warning: low value for upper Taper-radius cutoff:%f\n", swb);
	...
	control->Tap4 = -35.0 * (swa3 + 9.0 * swa2 * swb + 9.0 * swa * swb2 + swb3)
			/ d7;
...
char Read_Force_Field(FILE* fp, reax_interaction* reax, output_controls* out_control) {
	...
	reax->sbp = (single_body_parameters*) calloc(reax->num_atom_types,
			sizeof(single_body_parameters));
	reax->tbp = (two_body_parameters**) calloc(reax->num_atom_types,
			sizeof(two_body_parameters*));
	reax->thbp = (three_body_header***) calloc(reax->num_atom_types,
			sizeof(three_body_header**));
	reax->hbp = (hbond_parameters***) calloc(reax->num_atom_types,
			sizeof(hbond_parameters**));
	reax->fbp = (four_body_header****) calloc(reax->num_atom_types,
			sizeof(four_body_header***));
	tor_flag = (char****) calloc(reax->num_atom_types, sizeof(char***));

	for (i = 0; i < reax->num_atom_types; i++) {
		reax->tbp[i] = (two_body_parameters*) calloc(reax->num_atom_types,
				sizeof(two_body_parameters));
		reax->thbp[i] = (three_body_header**) calloc(reax->num_atom_types,
				sizeof(three_body_header*));
		reax->hbp[i] = (hbond_parameters**) calloc(reax->num_atom_types,
				sizeof(hbond_parameters*));
		reax->fbp[i] = (four_body_header***) calloc(reax->num_atom_types,
				sizeof(four_body_header**));
		tor_flag[i] = (char***) calloc(reax->num_atom_types, sizeof(char**));

		for (j = 0; j < reax->num_atom_types; j++) {
			reax->thbp[i][j] = (three_body_header*) calloc(
					reax->num_atom_types, sizeof(three_body_header));
			reax->hbp[i][j] = (hbond_parameters*) calloc(reax->num_atom_types,
					sizeof(hbond_parameters));
			reax->fbp[i][j] = (four_body_header**) calloc(reax->num_atom_types,
					sizeof(four_body_header*));
			tor_flag[i][j] = (char**) calloc(reax->num_atom_types,
					sizeof(char*));

			for (k = 0; k < reax->num_atom_types; k++) {
				reax->fbp[i][j][k] = (four_body_header*) calloc(
						reax->num_atom_types, sizeof(four_body_header));
				tor_flag[i][j][k] = (char*) calloc(reax->num_atom_types,
						sizeof(char));
			}
		}
	}
	...
	for (i = 0; i < reax->num_atom_types; i++) {
		...
		reax->sbp[i].nlp_opt = 0.5 * (reax->sbp[i].valency_e
				- reax->sbp[i].valency);
	...
	for (i = 0; i < reax->num_atom_types; i++)
		for (j = i; j < reax->num_atom_types; j++) {
			...
			reax->tbp[i][j].r_pp = 0.5 * (reax->sbp[i].r_pi_pi
					+ reax->sbp[j].r_pi_pi);
			reax->tbp[j][i].r_pp = 0.5 * (reax->sbp[j].r_pi_pi
					+ reax->sbp[i].r_pi_pi);

			reax->tbp[i][j].p_boc3 = sqrt(reax->sbp[i].b_o_132
					* reax->sbp[j].b_o_132);
			reax->tbp[j][i].p_boc3 = sqrt(reax->sbp[j].b_o_132
					* reax->sbp[i].b_o_132);

			reax->tbp[i][j].p_boc4 = sqrt(reax->sbp[i].b_o_131
					* reax->sbp[j].b_o_131);
			reax->tbp[j][i].p_boc4 = sqrt(reax->sbp[j].b_o_131
					* reax->sbp[i].b_o_131);

			reax->tbp[i][j].p_boc5 = sqrt(reax->sbp[i].b_o_133
					* reax->sbp[j].b_o_133);
			reax->tbp[j][i].p_boc5 = sqrt(reax->sbp[j].b_o_133
					* reax->sbp[i].b_o_133);

			reax->tbp[i][j].D = sqrt(reax->sbp[i].epsilon
					* reax->sbp[j].epsilon);

			reax->tbp[j][i].D = sqrt(reax->sbp[j].epsilon
					* reax->sbp[i].epsilon);

			reax->tbp[i][j].alpha = sqrt(reax->sbp[i].alpha
					* reax->sbp[j].alpha);

			reax->tbp[j][i].alpha = sqrt(reax->sbp[j].alpha
					* reax->sbp[i].alpha);

			reax->tbp[i][j].r_vdW = 2.0 * sqrt(reax->sbp[i].r_vdw
					* reax->sbp[j].r_vdw);

			reax->tbp[j][i].r_vdW = 2.0 * sqrt(reax->sbp[j].r_vdw
					* reax->sbp[i].r_vdw);

			reax->tbp[i][j].gamma_w = sqrt(reax->sbp[i].gamma_w
					* reax->sbp[j].gamma_w);

			reax->tbp[j][i].gamma_w = sqrt(reax->sbp[j].gamma_w
					* reax->sbp[i].gamma_w);

			reax->tbp[i][j].gamma = POW(
					reax->sbp[i].gamma * reax->sbp[j].gamma, -1.5);

			reax->tbp[j][i].gamma = POW(
					reax->sbp[j].gamma * reax->sbp[i].gamma, -1.5);
	...
	for (i = 0; i < l; i++) {
		...
		if (j < reax->num_atom_types && k < reax->num_atom_types && m
				< reax->num_atom_types) {
	...
	for (i = 0; i < l; i++) {
		...
		{
			if (j < reax->num_atom_types && k < reax->num_atom_types && m
					< reax->num_atom_types && n < reax->num_atom_types) {
...
char Read_Control_File(FILE* fp, reax_system *system, control_params* control,
	...
	control->bboost = 0;
        control->bboost_nrad = 0;
	control->bboost_Vmax = 0.0;
	control->bboost_P1 = 0.0;
	control->bboost_q = 0.0;

	control->bias_con_de = 0;
        control->bias_con_de_atom1 = 0;
        control->bias_con_de_atom2 = 0;
	control->bias_con_de_cutoff = 0.3;
	...
	while (!feof(fp)) {
		...
		//}
		...
		} else if (strcmp(tmp[0], "run_amd") == 0) {
			ival = atoi(tmp[1]);
			control->amd = ival;
		} else if (strcmp(tmp[0], "amd_energy") == 0) {
			val = atof(tmp[1]);
			control->amd_energy = val;
		} else if (strcmp(tmp[0], "amd_alpha") == 0) {
			val = atof(tmp[1]);
			control->amd_alpha = val;
		} else if (strcmp(tmp[0], "amd_power") == 0) {
			val = atof(tmp[1]);
			control->amd_power = val;
		} else if (strcmp(tmp[0], "amd_func") == 0) {
			ival = atoi(tmp[1]);
			control->amd_func = ival;
			...
			ival = atoi(tmp[1]);
			control->bboost = ival;
		} else if (strcmp(tmp[0], "bboost_nrad") == 0) {
			ival = atoi(tmp[1]);
			control->bboost_nrad = ival;
		...
		} else if (strcmp(tmp[0], "bias_con_de_atom1") == 0) {
			ival = atoi(tmp[1]);
			control->bias_con_de_atom1 = ival;
		} else if (strcmp(tmp[0], "bias_con_de_atom2") == 0) {
			ival = atoi(tmp[1]);
			control->bias_con_de_atom2 = ival;
		} else if (strcmp(tmp[0], "bias_con_de_interval") == 0) {
			ival = atoi(tmp[1]);
			control->bias_con_de_interval = ival;
		} else if (strcmp(tmp[0], "bias_con_de_cutoff") == 0) {
			val = atof(tmp[1]);
			control->bias_con_de_cutoff = val;
